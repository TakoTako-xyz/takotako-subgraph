...
import { ProtocolData } from "./_mapping";
...
export function getOrCreateLendingProtocol(
  ...
  if (!lendingProtocol) {
    ...
    lendingProtocol.tokenPriceUSD = BIGDECIMAL_ZERO;
...
export function getAssetPriceInUSDC(
  ...
  if (equalsIgnoreCase(dataSource.network(), Network.TAIKO)) {
...
function snapshotPosition(position: Position, event: ethereum.Event): void {
  ...
  snapshot.blockNumber = event.block.number;
  snapshot.timestamp = event.block.timestamp;
  snapshot.sender = event.transaction.from;
  snapshot.save();
}
...
export function getOrCreateMarket(
  ...
  if (!market) {
    ...
    market.totalAllocPoint = BIGINT_ZERO;
    market.supplyAllocPoint = BIGINT_ZERO;
    market.borrowAllocPoint = BIGINT_ZERO;
