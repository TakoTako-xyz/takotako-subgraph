...
export function _handleReserveDataUpdated(
  ...
  if (market._sToken) {
    // the protocol does not have stable borrow rates
...
export function _handleDeposit(
  ...
  deposit.sender = event.transaction.from;
  deposit.save();
...
export function _handleWithdraw(
  ...
  withdraw.amountUSD = amount
    ...
    .div(exponentToBigDecimal(inputToken!.decimals))
    .times(market.inputTokenPriceUSD);
  withdraw.sender = event.transaction.from;
  withdraw.save();
...
export function _handleBorrow(
  ...
  borrow.amountUSD = amount
    ...
    .div(exponentToBigDecimal(inputToken!.decimals))
    .times(market.inputTokenPriceUSD);
  borrow.sender = event.transaction.from;
  borrow.save();
...
export function _handleRepay(
  ...
  repay.amountUSD = amount
    ...
    .div(exponentToBigDecimal(inputToken!.decimals))
    .times(market.inputTokenPriceUSD);
  repay.sender = event.transaction.from;
  repay.save();
...
export function _handleLiquidate(
  event: ethereum.Event,
  amount: BigInt, // The amount of collateral being liquidated
  marketId: Address, // The collateral market
  protocolData: ProtocolData,
  liquidator: Address,
  borrower: Address, // account liquidated
  debtToken: Address, // token repaid to cover debt,
  debtToCover: BigInt // The total amount liquidated
): void {
  const market = Market.load(marketId.toHexString());
  if (!market) {
  ...
  liquidate.debtMarket = repayTokenMarket.id;
  ...
  liquidate.debtAmountRepaid = debtToCover;
  liquidate.collateralTokenPriceUSD = market.inputTokenPriceUSD;
  liquidate.debtTokenPriceUSD = repayTokenMarket.inputTokenPriceUSD;
  liquidate.sender = event.transaction.from;
  liquidate.save();
